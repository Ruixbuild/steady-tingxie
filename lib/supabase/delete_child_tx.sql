...
-- Revision's own tables (revision_attempts, revision_mastery) use NO ACTION
-- instead, so a plain `delete from children` fails with a foreign-key
-- violation (23503) the moment any Revision data exists for that child.
-- This explicitly clears those first, then deletes the child row — it
-- doesn't touch any existing Revision logic/schema, it just respects that
-- data when a parent deletes a child profile (a core, non-Revision action).
-- (An earlier version of this file also deleted from revision_assignments —
-- a table from an abandoned, never-rebuilt Revision attempt that no
-- current feature references. Removed rather than left dangling.)
