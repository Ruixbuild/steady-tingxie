...
export default async function VocabLearnWordPage({
  ...
  const readRow = masteryRows.find((m) => m.skill === "read") ?? null;
  const writeRow = masteryRows.find((m) => m.skill === "write") ?? null;
  ...
  if (effectiveSkill === "read") {
    return (
      <ReadCard
        ...
        gridHref={`${base}/learn`}
        word={word}
        initialFlagged={readRow?.flagged ?? false}
      />
    );
  }
  ...
  return (
    <WriteCard
      ...
      gridHref={`${base}/learn`}
      word={word}
      initialFlagged={writeRow?.flagged ?? false}
    />
  );
}
